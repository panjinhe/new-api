...
func TestOaiResponsesCompletedResponseFromStream(t *testing.T) {
	resp := &http.Response{
		Body: io.NopCloser(strings.NewReader(strings.Join([]string{
			...
			``,
			`data: {"type":"response.completed","response":{"id":"resp_1","model":"gpt-5.2","usage":{"input_tokens":12,"output_tokens":4,"total_tokens":16}}}`,
			``,
			`data: [DONE]`,
			``,
		}, "\n"))),
	...
	if completed.Model != "gpt-5.2" {
		t.Fatalf("expected model gpt-5.2, got %q", completed.Model)
	}
